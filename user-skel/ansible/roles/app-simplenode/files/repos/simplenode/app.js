...
var init = function (newBuildNumber) {
  ...
  if (!namespace || namespace.length == 0 || namespace == EMPTY) {
    ...
    else if (cloud_automation_stage && pod_name.length) namespace = pod_name;
  }
  ...
  if (!inProduction) {
    inProduction =
      os.hostname().includes("green") || os.hostname().includes("blue");
    if (!inProduction) {
  ...
  console.log(
    "Init: " + buildNumber + "/" + failInvokeRequestPercentage + "/" + minSleep
  );
};
...
var log = function (severity, entry) {
  if (severity === SEVERITY_DEBUG) {
    ...
    var logEntry =
      new Date().toISOString() + " - " + severity + " - " + entry + "\n";
    ...
    if (req.method === "POST") {
      ...
      if (url.pathname === "/api/invoke") {
        ...
        if (failInvokeRequestPercentage > 0) {
          invokeRequestCount++;
          var failRequest =
            invokeRequestCount % (100 / failInvokeRequestPercentage);
          console.log(
            invokeRequestCount +
              "%" +
              failInvokeRequestPercentage +
              "=" +
              failRequest
          );
          if (failRequest == 0) {
        ...
        var result = http.get(urlRequest, function (getResponse) {
          ...
          log(
            SEVERITY_DEBUG,
            "HEADERS: " + JSON.stringify(getResponse.headers)
          );
      ...
      if (url.pathname === "/api/version") {
        if (
          url.query["newBuildNumber"] &&
          url.query["newBuildNumber"] != null
        ) {
          var newBuildNumber = url.query["newBuildNumber"];
          log(
            SEVERITY_WARNING,
            "Somebody is changing! buildNumber from " +
              buildNumber +
              " to " +
              newBuildNumber
          );
      ...
      if (url.pathname === "/api/cpuload") {
        ...
        if (reqPerMin <= 45) {
          sleepTime =
            (Math.pow(reqPerMin, 2) - Math.pow(reqPerMin, 3) / 100) / 2;
        }
      ...
      var finalHtml = html
        .replace("BACKGROUND-COLOR", getBackgroundColor())
